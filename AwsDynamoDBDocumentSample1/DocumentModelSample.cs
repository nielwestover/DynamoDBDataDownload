...
namespace AwsDynamoDBDocumentSample1
{
	...
	public partial class Program
    {
        public static void RunDocumentModelSample(AmazonDynamoDBClient client)
        {
			...
			//var l = context.Load<levelInfo>("all");
            Console.WriteLine("Loading Businesses table");
            Table table = Table.LoadTable(client, "levelInfo");
			Document doc = table.GetItem("all");
			var l = doc["levels"].AsListOfDocument();
			Table packs = Table.LoadTable(client, "packLevels");
			List<Level> levels = new List<Level>();
			for (int i = 0; i < l.Count; ++i)
			{
				var vals = l[i].Values.GetEnumerator();
				vals.MoveNext();
				string name = vals.Current;
				vals.MoveNext();
				string numLevels = vals.Current;
				int num;
				Int32.TryParse(numLevels, out num);
				for (int j = 0; j < num; ++j)
				{
					string key = name + "_" + j;
					Level level = context.Load<Level>(key);
					levels.Add(level);
				}

			}
			string allLevels = Newtonsoft.Json.JsonConvert.SerializeObject(levels);
			File.WriteAllText("D:/allLevelsOutput.txt", allLevels);
